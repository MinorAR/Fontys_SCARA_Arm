...
const double WorkspaceDistance = 0.5;
  ...
  for (volatile double z = Zmin; z <= Zmax; z = z + Zresolution )
  {
	...
	case 0:
		for (x = Xmin ; x <= Xmax; x = x + Xresolution )
			...
			arrow.id = size + 2; //add 2 because id 1 is already used.
			arrow.points.push_back(PathPoints0);
			//path_pub.publish( arrow );	

			PathPoints0_1.x = 0.0;//normals->points[ pointIdxNKNSearch[i] ].normal_x;
			PathPoints0_1.y = 0.0;//normals->points[ pointIdxNKNSearch[i] ].normal_y;
			PathPoints0_1.z = 0.0;//normals->points[ pointIdxNKNSearch[i] ].normal_z;	

			arrow.points.push_back(PathPoints0_1);
			path_pub.publish( arrow );	

			//PathArrow[size].orientation.x = normals->points[ pointIdxNKNSearch[i] ].normal_x;
			//PathArrow[size].orientation.y = normals->points[ pointIdxNKNSearch[i] ].normal_y;
			//PathArrow[size].orientation.z = normals->points[ pointIdxNKNSearch[i] ].normal_z;
	...
	case 1:
			...
			PathPoints1_1.x = 0.0;//normals->points[ pointIdxNKNSearch[i] ].normal_x;
			PathPoints1_1.y = 0.0;//normals->points[ pointIdxNKNSearch[i] ].normal_y;
			PathPoints1_1.z = 0.0;//normals->points[ pointIdxNKNSearch[i] ].normal_z;
			arrow.points.push_back(PathPoints1_1);
			path_pub.publish( arrow );	
...
//	arrow.pose.position.x = PathArrow[i].position.x;
//	arrow.pose.position.y = PathArrow[i].position.y;
//	arrow.pose.position.z = PathArrow[i].position.z;
