...
void cloud_cb(const pcl::PCLPointCloud2ConstPtr& input)
{
  ...
  bool start;
  nh.getParam("/pcl_capturing/finished" , start);
  bool finished;
  nh.getParam("/pcl_filtering/finished" , finished);

  if (start == false && finished == false)
  {
	std::cerr << "pcl_filtering: start signal NOT received" << std::endl;
  	return;
  }
  else if (cloud_converted == 0)
  {
        std::cerr << "pcl_filtering: input is empty" << std::endl;
	return;
  }
  else
  {
        std::cerr << "pcl_filtering: start" << std::endl;
  }
  ...
  std::cerr << "PointCloud after filtering: " << cloud_voxel->width * cloud_voxel->height << " data points." << std::endl;

  // Publish the data
  pub.publish(cloud_voxel);
  ros::param::set("/pcl_filtering/finished", true);
